...
const handleSearch = async (type = searchType, value = query) => {
  if (!value) return;
  setLoading(true);
  let url;
  ...
  try {
    ...
    // Get full meal details if filter response is minimal
    if (['category', 'ingredient', 'area'].includes(type)) {
      const meals = data.meals || [];

      const fullMealPromises = meals.map(meal =>
        fetch(`${API}/lookup.php?i=${meal.idMeal}`).then(res => res.json())
      );
        ...
        <main className="px-4 py-8 max-w-7xl mx-auto">
            ...
            <div className="mb-10">
                <div className="flex items-center justify-between mb-2">
                    <h2 className="font-bold text-xl text-gray-800">🧂 Filter by Ingredient</h2>
                    <button
                    onClick={() => setShowIngredients(!showIngredients)}
                    className="text-sm text-[#D00000] font-medium hover:underline flex items-center gap-1 transition-colors"
                    >
                ...
                <div
                    ...
                    showIngredients ? 'max-h-64' : 'max-h-0'
                    }`}
                >
                    <div className="overflow-y-auto max-h-64 border rounded-lg p-4 bg-white shadow-md custom-scrollbar">
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                        {ingredients.map(ing => (
                        <button
                            key={ing.strIngredient}
                            onClick={() => handleSearch('ingredient', ing.strIngredient)}
                            className="bg-gray-100 hover:bg-[#D00000] hover:text-white text-xs font-semibold px-3 py-2 rounded-full transition-all duration-200 shadow-sm"
                        >
                            {ing.strIngredient}
                        </button>
                        ))}
